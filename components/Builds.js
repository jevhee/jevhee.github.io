  ...
  <div class="flex flex-col">
    <ul class="list-none pl-0 m-0">
      ${data.projects.map(project => `
        <li class="relative mb-8 last:mb-0 text-base text-macchiato-text flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-center justify-between border-b border-macchiato-surface0/50 pb-6 last:border-0 last:pb-0">
          ...
          <div class="flex-1 flex flex-col">
            ...
            <div class="flex flex-wrap items-center gap-x-2.5 gap-y-1 mt-1 text-[13.5px] text-macchiato-subtext0 font-medium">
              ${project.stack.map((tech, index) => `<span>${tech}</span>${index < project.stack.length - 1 ? '<span class="opacity-40 text-[10px]">●</span>' : ''}`).join('')}
            </div>
          </div>
